...
func (cq *CategoryQuery) QueryRooms() *RoomQuery {
...
func (cq *CategoryQuery) WithRooms(opts ...func(*RoomQuery)) *CategoryQuery {
	query := (&RoomClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withRooms; query != nil {
		if err := cq.loadRooms(ctx, query, nodes,
			func(n *Category) { n.Edges.Rooms = []*Room{} },
			func(n *Category, e *Room) { n.Edges.Rooms = append(n.Edges.Rooms, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadRooms(ctx context.Context, query *RoomQuery, nodes []*Category, init func(*Category), assign func(*Category, *Room)) error {
	...
	query.Where(predicate.Room(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.RoomsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.category_rooms
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_rooms" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_rooms" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
