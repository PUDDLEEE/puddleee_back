...
// QueryQuestioner chains the current query on the "questioner" edge.
func (rq *RoomQuery) QueryQuestioner() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, room.QuestionerTable, room.QuestionerColumn),
		)
...
// QueryRespondent chains the current query on the "respondent" edge.
func (rq *RoomQuery) QueryRespondent() *UserQuery {
	query := (&UserClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(room.Table, room.FieldID, selector),
			sqlgraph.To(user.Table, user.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, room.RespondentTable, room.RespondentPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(rq.driver.Dialect(), step)
		return fromU, nil
	}
...
// WithQuestioner tells the query-builder to eager-load the nodes that are connected to
// the "questioner" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoomQuery) WithQuestioner(opts ...func(*UserQuery)) *RoomQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withQuestioner = query
	return rq
}

// WithRespondent tells the query-builder to eager-load the nodes that are connected to
// the "respondent" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoomQuery) WithRespondent(opts ...func(*UserQuery)) *RoomQuery {
	query := (&UserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRespondent = query
	return rq
}
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withQuestioner; query != nil {
		if err := rq.loadQuestioner(ctx, query, nodes, nil,
			func(n *Room, e *User) { n.Edges.Questioner = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withRespondent; query != nil {
		if err := rq.loadRespondent(ctx, query, nodes,
			func(n *Room) { n.Edges.Respondent = []*User{} },
			func(n *Room, e *User) { n.Edges.Respondent = append(n.Edges.Respondent, e) }); err != nil {
			return nil, err
		}
	}
...
func (rq *RoomQuery) loadQuestioner(ctx context.Context, query *UserQuery, nodes []*Room, init func(*Room), assign func(*Room, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Room)
	for i := range nodes {
		if nodes[i].user_own_rooms == nil {
			continue
		}
		fk := *nodes[i].user_own_rooms
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_own_rooms" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RoomQuery) loadRespondent(ctx context.Context, query *UserQuery, nodes []*Room, init func(*Room), assign func(*Room, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(room.RespondentTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(room.RespondentPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(room.RespondentPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(room.RespondentPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "respondent" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
